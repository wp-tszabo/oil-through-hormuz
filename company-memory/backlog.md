...
| Item | Owner | Notes |
|---|---|---|
| **Final go-ahead to merge PR #4 (= publish)** | Owner | The headline question is **answered** (see Done) and the page is rebuilt to the owner's decision, self-checked PASS, and pushed to `copy-review/homepage-v2`. The owner's comment resolved *what the page says*; it did not say "merge", and merging is what fires the Pages deploy. One word on PR #4 publishes it. |
| **Two small copy confirmations** | Owner | Still open from the last cycle, and neither blocks the merge: (a) the name to put in the copyright line, currently "the site operator"; (b) confirmation that "Strait" (not "Straight") is wanted — "Strait" is the correct spelling of the waterway and is what the page uses. |
...
|---|---|---|---|
| **Decide the headline: measured quarter, honest range, or daily number** | Owner (decision) / CEO (implementation) | 2026-09-17 (4th cycle) | **Decided by the owner on PR #4 — the daily model estimate is the headline, with "estimated" in the title.** Overrules the CEO's recommendation, which is the escalation working rather than failing. Implemented and pushed to `copy-review/homepage-v2`: estimate labelled in four places, band 1.5–6.9 m b/d shown under the figure per rubric §1.6, methodology section rewritten in plain English, Terms/Copyright corrected because they no longer described a third-party figure. Rubric PASS. Critical issue #5 resolved. **Publishing is a separate approval and is still open.** |
