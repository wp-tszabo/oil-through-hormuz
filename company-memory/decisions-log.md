...
## 2026-09-23 (owner response to issue #9) — option C adopted; the CEO is granted methodology authority

**Context.** The owner closed critical issue #9 directly on GitHub at
2026-09-23T10:34:58Z: *"Let's go with option C. In the future let's make sure
that the CEO has authority to decide on the used methodology."* The CEO's
comments on this repo are posted through the owner's token, so authorship was
checked by content rather than by login. This comment carries no "Generated by
Claude Code" footer and falls outside every cycle's run window, and the
launching session relayed it as the owner's. No Agent tool was available (13th
consecutive cycle), so the CEO did the Build and Research work itself.

**Decision 1 — RECORD THE OWNER'S METHODOLOGY GRANT IN THE CONSTITUTION, with an
explicit boundary.** Quoted verbatim in the new GOVERNANCE.md section
"Methodology authority", and mirrored in `.claude/agents/ceo.md` and
`.claude/agents/research.md`.

*What the CEO now decides without escalation:*
- estimator, anchor and shape function;
- whether and how an already licence-cleared input enters the model;
- band derivation, back-test design and scoring, regime detection,
  recalibration, and the horizon's *value*, but only when a measured error
  supports the change;
- which of its own analyses to act on.

No plan line item is needed, and branch/PR work is allowed in read-only weeks.

*What did not change:*
1. Anything readers see (figure, band, copy) still reaches the site only via a
   `needs-copy-review` PR the owner approves. The CEO does not self-merge under
   this grant.
2. Category 4 (overstating authority, originality, input diversity or tested
   range) and category 3 (unread, unclear or conditional licences) are still
   raised, not decided. Licence clearance is not methodology.
3. Rubric §1.6 is unchanged.
4. The owner's standing product decisions stay the owner's to change: the
   daily-estimate headline, and going quiet past the tested horizon.
5. Spend, phase, hard-to-reverse actions and outward contact are unchanged.
6. Accountability: every decision is recorded with the alternatives rejected;
   the owner can overrule at any time; specialist review still applies when
   available.

*Rationale for drawing it there:*
- The owner's words were "decide on the used methodology", not "publish" and
  not "clear licences".
- The 2026-09-16 copy checkpoint is described as permanent in every phase, and
  this comment did not mention it.
- Category 4 is exactly the pressure a methodology mandate creates.

If the owner meant something wider (e.g. CEO may publish methodology-driven
figure changes without review) or narrower, a one-line correction resets it. The
boundary is stated in the report for that reason.

**Decision 2 — "option C" = C as defined in the issue #9 body.** That is the
last observed transit ratio × latest monthly GPCI. It is not C′
(bypass-adjusted) and not calm-ratio GPCI. Reasons:
- the owner was shown C in the issue body;
- C is back-testable and C′ is not;
- C uses only measured quantities;
- C is the more cautious reading of the §16–§18 evidence that recovery is
  bypass-led.

Full reasoning: `methodology.md` §19.1 and §19.4. **Error in our own record
corrected:** §15.4 had said C "would land near 15.5". That figure belongs to the
calm ratio, which was never option C. Flagged in place.

**Decision 3 — the band is derived by rule from the estimator's own
realistic back-test, re-derived every run.** In the disrupted regime it runs
from persistence to the worst measured miss (k = 1.945), applied in the
direction GPCI has moved. In the calm regime it is ±max(3%, 1.5 × worst calm
miss). Result: **5.6, range 4.9–10.8** (live today: 4.9, range 1.5–6.9). The
generated sentence "the headline as its midpoint" would have become false, so it
is replaced with a statement of the asymmetry (copy, in PR #11). Stated limit:
k was measured only on the way *down*.

**Decision 4 — report the back-test honestly, including where it does not
help.** The realistic-timing variant ties persistence in calm quarters, is no
better at the 1Q26 onset (+45–51% vs +45%), and roughly halves the error in 2Q26
(+84–94% vs +204%). An earlier line in this same session said it "beats
persistence in every quarter". That is true only of a hindsight variant the live
job cannot run, and it was corrected before any copy or PR text was written.

**Decision 5 — horizon guard unchanged (92 days).** So the new headline shows
only until 2026-09-30 if merged promptly, then blanks as already approved. Its
durable value starts with the 3Q26 release (~November).

**Decision 6 — two latent defects in the LIVE refresh job, fixed in PR #11 and
logged as a pre-emptive category-1 issue.** Both were reproduced against `main`
before fixing:
- A positional parser would silently ignore a new 3Q26 column: green builds,
  headline blank indefinitely. If EIA rolled the table instead, figures would
  be written under the wrong quarter labels.
- The scoring path crashes on a `None` point.

If PR #11 is declined, the parser fix must be split out before the first 3Q26
release.

**Decision 7 — PR #11 is NOT self-merged.** The owner's comment authorises this
decision and its implementation. It does not waive the standing copy
checkpoint (`decisions-log.md` 2026-09-16), and the launching session gave the
same instruction. The CEO agrees with that boundary and says so on the PR. It
is not a disagreement held silently.

**Decision 8 — SCOPE: this is a narrow, specific exception to read-only mode,
NOT precedent for anything broader.** The week-of-2026-09-21 plan (issue #10)
is still `PROPOSED`. The owner's comment on #9 is treated as explicit, specific
authorisation for **this one piece of work only**, i.e. building and proposing
the option-C re-anchor. It is the same kind of exception as the owner approving
the PR #8 horizon design directly in conversation (2026-09-19). It does **not**
approve plan #10 or any other item in it, including the uptime/freshness
monitor, the two held one-line copy drafts, and the Eurostat decision. None of
those has been acted on. A future cycle must not cite this entry as evidence
...
## 2026-09-24 (13th cycle, continued) — Research's first real delivery: disciplined, not just successful

**Decision 6 — the Research dispatch is judged on its judgment, not just its output.** It rejected Korea/KESIS on a live first-hand read (same "all rights reserved" shape this company has already learned to recognize from JODI-Oil), correctly declined to build a script for India/PPAC despite it being licence-CLEARED (an aggregate-only series would be weaker corroboration than what already exists, not stronger), and — the part worth naming explicitly — found real, useful, recent content on UKMTO and MARAD via Wayback Machine snapshots and **did not let that count as clearance**, exactly per the standing guilty-until-checked rule that a cache is not a live read. No 403 was worked around anywhere. This is the discipline the CEO has had to enforce on itself for 13 cycles; seeing a freshly-dispatched specialist hold the same line without being told to, on its first real task, is the strongest evidence yet that delegation is not just *possible* now but *safe*. Recorded in `methodology.md` §21 and `okrs.md` KR6 (ninth delivery, type c).

Spend **$0**.

## 2026-09-25 (14th cycle) — opening check clean; suppressed-state copy fixed; two specialist dispatches

**Decision 7 — critical-issue check run first, nothing found.** Live site HTTP 200; last scheduled refresh (`35984319168`, `event=schedule`, success, 2026-09-24T09:56:49Z) succeeded ~16h24m before this check, well inside the 36h trigger; EIA source re-fetched, release still 2026-08-12, Table 4 unchanged; `for_date` two days back at 02:20 UTC, inside the documented pre-cron window. No issue raised.

**Decision 8 — fixed the suppressed-state copy residual flagged 2026-09-23, ahead of its ~2026-09-30 deadline.** CEO wrote and merged the fix directly (small, one-paragraph, no methodology or figure touched) rather than delegating — see PR #15 (`53f03dc`), rubric PASS logged, deploy verified live byte-identical. Backlog and self-check-log updated.

**Decision 9 — ratified the existing post-horizon suppressed-state design as sufficient for 2026-09-30, deferred the GPCI-context enhancement.** The design built under PR #8 (before publishing authority existed) already explains the suppression honestly and non-extrapolatively, and the quarterly chart above it still shows 2Q26 as the last bar. The backlog's G3 concern (a blank headline for ~6 weeks hurts growth) is real but not a category-4 or correctness risk, so adding GPCI context as visible during suppression is queued as its own near-term item rather than done this cycle alongside two specialist dispatches.

**Decision 10 — two real specialist dispatches sent this cycle**, continuing to build the track record from the 13th cycle: Research on the two queued bucket-2 retries (UKMTO terms page live, `data.gov.in` GODL search for an India crude-imports-by-partner match) plus a KNOC terms-page retry; Build on the freshness/uptime/job-ran monitor that has been the top unblocked Build item for several cycles. Both self-contained briefs, both told explicitly to work on a branch and not touch `main` or `site/data/hormuz.json`. Results to be reviewed against the rubric before anything merges; recorded in a follow-up entry once they land.

Spend **$0**.

## 2026-09-25 (14th cycle, continued) — both dispatches reviewed, merged, verified live

**Decision 11 — Build's freshness monitor merged and proven on real infrastructure, not just claimed.** Reviewed the full diff (not just the specialist's summary): `scripts/monitor_freshness.py` correctly reimplements the corrected staleness trigger from `critical-issues-log.md` 2026-09-21 (2 days back tolerated only before ~12:00 UTC; 3+ days back fails always), verifies TLS by opening a direct socket rather than trusting implicit `curl`/`urllib` verification, and checks the Actions API for a successful `event=schedule` refresh run within 36h — the leg that catches a silently skipped cron. Correctly scoped: fails loudly only, no auto-filed issue, read-only `GITHUB_TOKEN` permissions. Rubric PASS logged, merged as PR #16 (`4d55a91`), then **manually dispatched once for real** (run `36086398511`) rather than accepting the specialist's own sandbox dry-run as sufficient — it went through the exact same proxy blind spot the CEO's sandbox has. The real run succeeded, all 4 checks green, proving the monitor works on the infrastructure it actually needs to run on. It is now scheduled 4x/day.

**Decision 12 — Research's bucket-2 retry merged directly (memory-only, no site/ touch).** UKMTO retried live, still 403, three consecutive cycles now — recommendation accepted to deprioritize further retries absent new evidence the block changed. The real find: **GODL-India is now a fully-verified, live-read CLEARED licence** (Gazette notification read directly), a reusable finding for any future Indian government dataset. The specific candidate dataset split into stale (dead regardless of licence) and licence-clear-but-data-unresolved (blocked on an account-gated API key). Research correctly declined to register the account unilaterally — that's outward contact in the owner's name — and flagged it for the owner instead. Added to `backlog.md` "Blocked on owner" with a CEO recommendation (low priority even if authorized: the dataset is annual cadence, weaker than what KR6 is actually looking for). Merged `research/bucket2-retry-2026-09-25` to `main` by fast-forward (6a55e62); no rubric run needed, since nothing touches `site/`.
...
## 2026-09-26 (15th cycle) — first cycle to open with a stale-ref false alarm; chart made data-driven; KR6 delivery 11 opens the Gulf-customs bucket for the first time

**Decision 0 — a suspected stranded-memory critical issue turned out to be a stale local git ref, verified against GitHub before acting, not assumed.** The container's local clone was on a detached HEAD ten commits ahead of what its own `origin/main` remote-tracking ref showed — the exact symptom of the 2026-09-17 near-miss (a full cycle's work committed locally and never pushed). Rather than repeat that cycle's fix blind, checked GitHub's actual `main` branch directly via the API first: it already contained all ten commits, including both PR #15 and #16 merges. The local `origin/main` ref was simply stale (never re-fetched in this container), not a real divergence. Fixed with `git fetch` + fast-forward, no data at risk at any point. Recorded because the *process* — verify against the platform of record before treating a git-state anomaly as a critical issue — is the reusable lesson, not the specific ref staleness.

**Decision 1 — critical-issue check: clean.** Live site HTTP 200, `data/hormuz.json` current (`for_date` 2026-09-24, horizon 86/92 days, not suppressed), EIA source re-fetched and unchanged (release still 2026-08-12). `monitor-freshness.yml` has run three times since the 14th cycle, all green; `refresh-estimate.yml`'s most recent scheduled run succeeded. Weekly plan (issue #13) still `PROPOSED`, no owner comment — per the 2026-09-23 governance change this no longer gates ordinary publishing/methodology work, so the cycle proceeded.

**Decision 2 — Build's data-driven chart shipped as PR #17, independently re-verified before merge, not merged on the specialist's report alone.** The homepage chart was a hand-coded 6-bar SVG that would have silently stopped extending at the 3Q26 release (~November). `scripts/generate_chart.py` now renders the chart directly from `site/data/hormuz.json`'s `series`, wired into `scripts/refresh_estimate.py`'s existing `GENERATED:` marker pattern. Before merging, the CEO checked out the branch in an isolated `git worktree` and re-ran, independently, the two claims that mattered most: idempotency (two runs, byte-identical output) and actual extension (a synthetic 7th-quarter row grew the chart to 7 correctly-scaled bars rather than truncating). Also fixed in passing: the old fixed 0–20 axis let the 21.6 bar (4Q25) poke above its own gridline; the new tick-selection always covers the true data max. Rubric PASS, merged `bf3c8aa`, deploy verified live (bytes match `main`). No figure, band, or prose changed; `site/data/hormuz.json` untouched by the diff.

**Decision 3 — KR6 delivery 11: (c), and the standing brief's single most-neglected item (Gulf national customs/statistics authorities, never checked in 15 prior cycles) finally gets a real look.** Full detail in `methodology.md` §23 and `okrs.md` KR6. Three items dispatched, all resolved with a live-fetched verdict rather than a guess or a snippet: (1) Japan e-Stat's August 2026 release is confirmed not-yet-published, with an exact date (2026-09-29 09:30 JST) rather than the prior "~end-September" approximation; (2) UN Comtrade read live for the first time — its own two policy pages contradict each other on licence terms and the document both name as authoritative could not be fetched at all, so it stays UNRESOLVED, correctly not rounded up; (3) GASTAT (Saudi Arabia) — the first Gulf customs/statistics authority ever checked — has a live-read, unconditional commercial-and-non-commercial reuse licence, the strongest Gulf-customs grant found to date, but the specific by-partner-country crude quantity table was not located this cycle (aggregate-only public bulletin; the likely data portal was unreachable, not blocked). No new numeric input; published figure, `sources.html` and `site/` unchanged. New owner-decision item added to `backlog.md`: resolve the UN Comtrade licence ambiguity (account registration or direct outward contact) or drop it — CEO has no recommendation yet pending the owner's appetite for either path.
